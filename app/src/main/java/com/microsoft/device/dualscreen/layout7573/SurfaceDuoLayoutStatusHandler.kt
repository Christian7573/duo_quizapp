...
package com.microsoft.device.dualscreen.layout7573
...
class SurfaceDuoLayoutStatusHandler internal constructor(
    ...
    init {
        val lol =
            ScreenHelper.isDualMode(
                activity
            );
        System.out.println(lol);
        System.out.println("HELLLLLLLOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO");
        if (ScreenHelper.isDualMode(
                activity
            )
        ) {
            System.out.println("Why the heck are we here");
            ...
            if (singleScreenLayout != -1) {
                System.out.println("Let me debug here ploeas");
    ...
    private fun addViewsDependingOnScreenMode() {
        screenMode = if (ScreenHelper.isDualMode(
                activity
            )
        ) {
    ...
    internal fun onConfigurationChanged(
        ...
        newConfig?.let {
            if (it.orientation == Configuration.ORIENTATION_PORTRAIT) {
                // DOUBLE_LANDSCAPE
                if (ScreenHelper.isDualMode(
                        surfaceDuoLayout.context as Activity
                    ) &&
    ...
    private fun refreshDualScreenContainersState(surfaceDuoLayout: SurfaceDuoLayout, orientation: Int, containerId: Int) {
        ...
        val screenRectangles =
            ScreenHelper.getScreenRectangles(
                activity
            )
